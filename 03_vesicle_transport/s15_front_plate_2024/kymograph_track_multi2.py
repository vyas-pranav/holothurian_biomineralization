...
"""Draw regions on a kymograph and extract one vesicle track per region (Tk GUI, 2024 version).

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Fig. S15 (all 93 front-plate tracks were extracted with this version,
                from the 'Reslice (AVRG) of images.tif' kymographs)
What it does  : Same interface as ../kymograph_track_multi3.py (open a kymograph, paint
                one region per vesicle trace, 'Process All Tracks'), but in every time row
                of a region the track point is the intensity-weighted centroid of the
                brightest run of pixels above the row's 80th percentile (CLAHE-enhanced).
Inputs        : a kymograph chosen in the file dialog (S15: video1/.../track_<k>/kymograph/
                and video2/track_<k>/kymograph/ 'Reslice (AVRG) of images.tif')
...
Run           : python kymograph_track_multi2.py   (interactive Tk window; never run unattended)
"""
...
class KymographAnnotator:
    ...
    def process_track(self, mask, track_id):
        ...
        # Identify the midpoint of the brightest region in each row
        track_points = []
        for y in range(masked_region.shape[0]):
            row = enhanced_region[y, :] * (mask_array[y, :] > 0)  # Consider only pixels within the mask

            # Skip rows without any pixels in the mask
            if np.sum(row) == 0:
                continue

            # Threshold to isolate bright regions
            threshold_value = np.percentile(row[row > 0], 80)
            binary_row = row > threshold_value

            # Label contiguous regions in the row
            labeled_regions = measure.label(binary_row.reshape(1, -1))  # Reshape to 2D
            regions = measure.regionprops(labeled_regions, intensity_image=row.reshape(1, -1))

            # If bright regions exist, find the brightest one and get its midpoint
            if regions:
                total_intensities = [region.intensity_image.sum() for region in regions]
                brightest_region = regions[np.argmax(total_intensities)]
                midpoint_x = int(round(brightest_region.weighted_centroid[1]))
                track_points.append((midpoint_x, y))
