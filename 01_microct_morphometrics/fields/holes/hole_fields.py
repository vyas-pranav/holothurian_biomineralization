...
"""Holes of an ossicle coloured by area.
...
What it does  : Cuts the fitted base surface of ossicle_1 (loc_file_id[0]) into hole
                patches along the minimum-cycle loops of area_mid_base_G and colours
                each patch by its area (plasma, 0-0.001 mm^2) over the translucent
                ossicle.
...
def load_graphs(ii):
    ...
    return full_G, clean_full_G, simple_G, clean_simple_G, area_mid_G, full_base_G, clean_simple_base_G, area_mid_base_G
...
max_val = 0
min_val = 0
for jj in [0]: #range(n_files):      print(ii, files[ii])

    ii = loc_file_id[jj]
    ossicle, voxel_vol, skeleton, cl_skeleton = load_data(ii)

    if len(skeleton.lines())!=0 and len(cl_skeleton.lines())!=0:
        full_G, clean_full_G, simple_G, clean_simple_G, area_mid_G, full_base_G, clean_simple_base_G, area_mid_base_G = load_graphs(ii)
        G = area_mid_base_G
        ...
        for surf in hole_mesh_list:
            ...
            ctr += 1
    # # pl.add_mesh()
        print(pv_hole_list[ctr-1].active_scalars_name)
