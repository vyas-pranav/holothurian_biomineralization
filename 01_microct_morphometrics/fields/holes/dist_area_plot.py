...
"""Hole area vs distance from the centre.
...
What it does  : Pools (distance, area) of the pillared tables, other plates and foot
                pads (foot-pad CSVs, um converted to mm), bins distance in 0.005 mm bins
                and plots mean +/- SD with the raw points.
Inputs        : microct/animal_1/extracted_ossicles/Data
                Calculated/hole_distance_area/*.pickle, footpad/*.csv (from
                02_image_morphometrics_2d/footpad),
                microct/animal_1/extracted_ossicles/Data Calculated lists
Outputs       : hole_area_vs_distance.{svg,png} in the working directory
Environment   : environment-analysis.yml (Python 3.7)
Run           : python dist_area_plot.py
"""
...
for jj in list(range(len(active_loc_list))): #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if os.path.isfile(os.path.join(datadir, 'hole_distance_area', file_names[ii][0:-4]+'.pickle')):
        print('file exists')
        #load pickle file
        with open(os.path.join(datadir, 'hole_distance_area', file_names[ii][0:-4]+'.pickle'), 'rb') as f:
            data_whole_list = pickle.load(f)

        if len(data_whole_list) > 0:
            #get max value of distance and area from data_whole_list
            max_dist = max([data[0] for data in data_whole_list])
            max_area = max([data[1] for data in data_whole_list])

            #append data from data_whole_list to list area and dist lists
            for data in data_whole_list:
                whole_dist_list1.append(data[0])
                whole_area_list1.append(data[1])

#get rid of outliers in dist list with distance smaller than 0.01 and area smaller than 0.0001
remove_list = []
for ii in range(len(whole_dist_list1)):
    if whole_dist_list1[ii] < 0.01 and whole_area_list1[ii] < 0.0001:
        remove_list.append(ii)

whole_dist_list1 = [i for j, i in enumerate(whole_dist_list1) if j not in remove_list]
whole_area_list1 = [i for j, i in enumerate(whole_area_list1) if j not in remove_list]
    ...
    if os.path.isfile(os.path.join(datadir, 'hole_distance_area', file_names[ii][0:-4]+'.pickle')):
        ...
        if len(data_whole_list) > 0:
            ...
            for data in data_whole_list:
                ...
                whole_area_list2.append(data[1])



#get foot pad data
#import data from csv files in foot pad folder
foot_pad_folder = str(DATA_ROOT / "footpad")
#list all csv files in the folder
foot_pad_files = os.listdir(foot_pad_folder)
foot_pad_files = [f for f in foot_pad_files if f.endswith('.csv')]

#read all csv files and store data in two lists for dist and area
foot_pad_dist_list = []
foot_pad_area_list = []

for file in foot_pad_files:
    #read csv file
    foot_pad_data = pd.read_csv(os.path.join(foot_pad_folder, file))

    #extract dist and area data from csv file
    foot_pad_dist = foot_pad_data['Distance(um)'].tolist()
    foot_pad_area = foot_pad_data['Area (um2)'].tolist()

    #convert values into mm
    foot_pad_dist = [dist/1000 for dist in foot_pad_dist]
    foot_pad_area = [area/1000000 for area in foot_pad_area]

    #append data from csv file to list dist and area lists
    foot_pad_dist_list = foot_pad_dist_list + foot_pad_dist
    foot_pad_area_list = foot_pad_area_list + foot_pad_area
...
min_dist = min(whole_dist_list1)
max_dist = max(whole_dist_list1)

#create bins from range of distances from origin
# bin_edges = np.arange(-0.025, 1,0.05) 
bin_edges = np.arange(-0.0025, max_dist+0.005,0.005)
...
#find min and max values of distances from origin present in the second entry of all the tuples in data_whole_list
min_dist = min(whole_dist_list2)
max_dist = max(whole_dist_list2)
...
#get lists of lengths and thicknesses from data_whole_list
x = foot_pad_dist_list
y = foot_pad_area_list

#find min and max values of distances from origin present in the second entry of all the tuples in data_whole_list
min_dist = min(foot_pad_dist_list)
max_dist = max(foot_pad_dist_list)
...
#also mark points on the mean curve
plt.scatter(x, y, marker = 'o', color = color2, alpha = 0.1, s = 25, zorder = 2, linewidth = 0)
plt.scatter(bin_centers, bin_means, marker = 'o', color = color2, s = 100, zorder = 3, alpha = 0.5, label = 'foot pad', linewidth = 0)
...
plt.xlabel('Hole distance from center (in mm)') 
plt.ylabel(r'Hole area (in mm$^2$)')
...
#save figure
plt.savefig(os.path.join('hole_area_vs_distance.svg'), dpi = 600, bbox_inches = 'tight')
plt.savefig(os.path.join('hole_area_vs_distance.png'), dpi = 600, bbox_inches = 'tight')
