...
"""Hole area and hole distance from the origin (data producer).

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Hole-area panel (Fig. 2F)
What it does  : For each pillared table (selected_table) cuts the base surface into hole
                patches along the ordered minimum cycle basis of area_mid_base_G (outer
                boundary removed) and pickles, per hole, the distance of the patch
                centre of mass from the origin and the patch area. As in the original
                workflow, the other-plate pickles were produced by editing
                active_loc_list to selected_other_flat (this switch is not recorded in
                the code).
...
Outputs       : microct/animal_1/extracted_ossicles/Data
                Calculated/hole_distance_area/<ossicle>.pickle (data tree; read by
                dist_area_plot.py)
Environment   : environment-analysis.yml (Python 3.7)
Run           : python hole_dist_area.py
"""
...
def get_real_min_cycle_list(H, min_cycle_list, boundary_nodes):
    cycle_pos_list = []
    for cycle in min_cycle_list:
        cycle_pos = []
        for node in cycle:
    ...
    ordered_min_cycle_list.remove(real_boundary_nodes) 

    return ordered_min_cycle_list


def get_ordered_min_cycle_basis(G):
    ...
    ordered_real_min_cycle_list = get_real_min_cycle_list(G, min_cycle_list, boundary_nodes)

    return ordered_real_min_cycle_list
...
def get_hole_mesh_list(surface, min_cycle_basis, G):
    ...
    for cycle in min_cycle_basis:
        ...
        loop = pts     #order_points_by_angle(pts)
...
def assign_origin(G, origin_G):
    #find the origin node which is the max node id in bound_G
    origin = max(origin_G.nodes())

    #find node in G closest to origin
    origin_pos = origin_G.nodes[origin]['pos']
    dist = float('inf')
    for node in G.nodes():
...
#get a list of tuples (hole areas, hole distances) from the origin of the graph
def get_hole_dist_areas(hole_mesh_list, G, origin_G):
    #find origin of the graph
    origin = assign_origin(G, origin_G)
    origin_pos = G.nodes[origin]['pos']

    hole_dist_areas = []
    for mesh in hole_mesh_list:
        distance = np.linalg.norm(mesh.center_of_mass() - origin_pos)
        hole_dist_areas.append((distance, mesh.area()))

    return hole_dist_areas
...
for jj in list(range(len(active_loc_list))): #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if len(skeleton.lines())!=0 and len(cl_skeleton.lines())==0:
        ...
        pv_rem_recon = pv.wrap(rem_recon_mesh.polydata())#.subdivide(3, subfilter = 'loop')
        pv_graph =  graph_to_pv_mesh(G)    #.subgraph(min_cycle_basis_G[20])

        # map_to_skel_pt(pv_ossicle, pv_skeleton)


        min_cycle_basis_G = get_ordered_min_cycle_basis(G)
        ...
        data_whole_list = get_hole_dist_areas(hole_mesh_list, G, origin_clean_simple_base_G)
        #save data_whole_list as a pickle file
        with open(os.path.join(datadir, 'hole_distance_area', file_names[ii][0:-4]+'.pickle'), 'wb') as f:
...
#             pv_hole.set_active_scalars('area_value')
#             ctr+=1

#         cmap = mpl.cm.get_cmap('plasma')

#         pl.add_mesh(pv_ossicle, color = 'white', opacity = 0.5, diffuse = 0.9, smooth_shading = True, show_scalar_bar= True)#, clim = [0,0.13])#osc_color)#, specular=1.0, specular_power=10)
#         pl.add_mesh(pv_skeleton, cmap= cmap)
#         pl.add_mesh(pv_rem_recon, color = 'yellow', opacity =0.5,style = 'wireframe')    
#         # pl.add_mesh(pv_graph, color = 'black', render_lines_as_tubes=True, render_points_as_spheres = True, style='wireframe', line_width=50, point_size = 20, show_scalar_bar=False)
...
#         #show vedo mesh rem recon in vedo plotter
#         # plt3d.add(rem_recon_mesh)
...
#     distance_list.append(data[0])
#     area_list.append(data[1])

# sns.scatterplot(x=distance_list, y=area_list, s=100, color = 'black')
# plt.xlabel('Distance from Origin (in mm)')
# plt.ylabel('Hole Area (in mm\u00b2)')
# # plt.savefig('hole_area_dist.png', dpi = 300, bbox_inches='tight')
# plt.show()

# #create a plot to bin the distance from origin and plot the mean hole area with error bars
# fig, ax = plt.subplots()

# x = [data[0] for data in data_whole_list]
# y = [data[1] for data in data_whole_list]

# #find min and max values of distances from origin present in the first entry of all the tuples in data_whole_list
# min_dist = min([data[0] for data in data_whole_list])
# max_dist = max([data[0] for data in data_whole_list])
# #create bins from range of distances from origin
# bin_edges = np.arange(0, max_dist,0.002) 
...
# sns.lineplot(x=bin_centers, y=bin_means, color='black')
# # plt.fill_between(bin_centers, bin_means - bin_stdevs, bin_means + bin_stdevs, color='gray', alpha=0.2)
# #plot error bars
# plt.errorbar(bin_centers, bin_means, yerr=bin_stdevs, fmt='o', color='black',
#              ecolor='lightgray', elinewidth=3, capsize=0);
# #also mark points on the mean curve
# plt.scatter(bin_centers, bin_means, marker = '.', color = 'black')
