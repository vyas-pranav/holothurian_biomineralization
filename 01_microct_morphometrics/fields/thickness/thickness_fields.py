...
"""Medial-thickness map of an ossicle.
...
What it does  : Gives every mesh vertex of ossicle_206 (loc_file_id[205]) the mean
                medial thickness of its 100 nearest skeleton points and renders the
                ossicle coloured by medial thickness (plasma, 0-0.01 mm).
Inputs        : microct/animal_1/extracted_ossicles/aligned/{vtk,
                skeleton/<ossicle>/(vox.mhd, *skel_line.vtk, full_G, clean_full_G,
                simple_G, clean_simple_G, origin_clean_simple_G .gpickle)},
...
Run           : python thickness_fields.py
"""
...
def load_graphs(ii):
    ...
    return full_G, clean_full_G, simple_G, clean_simple_G, origin_clean_simple_G
...
def map_thick_to_skel_pt(mesh, skel):
...
max_val = 0
min_val = 0
for jj in [205]: #range(n_files):      print(ii, files[ii])

    ii = loc_file_id[jj]
    ossicle, voxel_vol, skeleton, cl_skeleton = load_data(ii)

    # if len(skeleton.lines())!=0 and len(cl_skeleton.lines())!=0:
    full_G, clean_full_G, simple_G, clean_simple_G, origin_clean_simple_G = load_graphs(ii)

    G = origin_clean_simple_G
    ...
    map_thick_to_skel_pt(pv_ossicle, pv_skeleton)
    ...
    # pl.add_mesh(pv_skeleton, cmap= cmap, line_width = 10)
