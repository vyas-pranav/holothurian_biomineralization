...
"""Medial thickness and path length from the origin for every skeleton node (data producer).

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Thickness panels (Fig. 2H)
What it does  : Puts the origin on the pillar-free full skeleton graph
                clean_full_base_G, computes for every node the Dijkstra path length from
                the origin along the skeleton (edge weight = Euclidean edge length) and
                pickles (node, path length, medial thickness) per ossicle. Active loop:
                jj = 98 (ossicle_99); the other ossicles were processed with the loop
                alternatives recorded in the comments.
...
Outputs       : microct/animal_1/extracted_ossicles/Data
                Calculated/thickness_data_lumped/<ossicle>.pickle (data tree; read by
                thickness_with_dist_lumped_scaled.py)
Environment   : environment-analysis.yml (Python 3.7)
Run           : python thickness_with_dist_lumped.py
"""
...
def get_dist_thickness_origin_to_all(G, bound_G):
    ...
    #get the thickness along the shortest path connecting origin and each node
    data_list = []
    for node in G.nodes():
...
active_loc_list = selected_other_flat[2:]

for jj in [98]: # [list(range(len(active_loc_list)))]: #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])

    kk = jj  #active_loc_list[jj] - 1
    ii = loc_file_id[kk]
    ...
    if len(skeleton.lines())!=0 and len(cl_skeleton.lines())==0:
        ...
        data_whole_list = get_dist_thickness_origin_to_all(clean_full_base_G, origin_clean_simple_base_G)
        #save data_whole_list as a pickle file
        with open(os.path.join(datadir, 'thickness_data_lumped', file_names[ii][0:-4]+'.pickle'), 'wb') as f:
...
# x = [data[1] for data in data_whole_list]
# y = [data[2] for data in data_whole_list]

# #find min and max values of distances from origin present in the second entry of all the tuples in data_whole_list
# min_dist = min([data[1] for data in data_whole_list])
# max_dist = max([data[1] for data in data_whole_list])
# #create bins from range of distances from origin
# bin_edges = np.arange(0, max_dist,0.01) 
...
# plt.xlabel('Path length from center (in mm)')
# plt.ylabel('Thickness (in mm)')
# plt.show()

# #create scatter plot of thickness vs distance values
# fig, ax = plt.subplots()
# ax.scatter([data[1] for data in data_whole_list], [data[2] for data in data_whole_list])
# ax.set_xlabel('Path length from center (in mm)')
# ax.set_ylabel('Thickness (in mm)')
# plt.show()
