...
"""Scaled, binned thickness vs path length of the foot pads (data producer for the Fig. 2H inset).
...
What it does  : Same scaling and binning as thickness_with_dist_lumped_scaled.py for the
                foot-pad graphs (the maximum path length is fixed to the value of the
                ossicle data set).
Inputs        : footpad/graphs/thick_data/thick_with_dist_lump_<graph>.pkl,
                footpad/graphs/test{1,2}graph_{simple_origin,full}.gpickle (foot-pad
                graphs from 02_image_morphometrics_2d/footpad)
Outputs       : footpad/graphs/thick_Data/thick_with_dist_lump_scaled_2<graph>.pkl
                (thick_Data = thick_data on Windows)
Environment   : environment-analysis.yml (Python 3.7)
Run           : python thickness_with_dist_lumped_scaled_foot.py
"""
...
for jj in [0]: #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if 1==2:
        ...
        data_file_name = graph_folder + '\\thick_data\\thick_with_dist_lump_' + simple_graph_file_names[jj] + '.pkl'
...
max_dist = max(max_dist1, max_dist1)
max_thick = max(max_thick1, max_thick1)

max_dist = 0.1379323110159021
...
for jj in [0]: #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if 1==2:
        ...
        with open(graph_folder + '\\thick_Data\\thick_with_dist_lump_scaled_2' + simple_graph_file_names[jj] + '.pkl', 'wb') as f:
            pickle.dump(data_lumped_scaled_list, f) 
