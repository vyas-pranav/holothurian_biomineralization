...
"""Scaled, binned thickness vs path length per ossicle (data producer for the Fig. 2H inset).
...
What it does  : For every pillared table and other plate divides the path lengths by the
                ossicle's own maximum, min-max normalises the medial thickness, bins the
                path length (bin width 0.002 mm divided by the largest path length of
                all ossicles, 69 bins) and pickles (bin centre, mean, SD) per ossicle.
Inputs        : microct/animal_1/extracted_ossicles/Data
                Calculated/{thickness_data_lumped/*.pickle, loc_file_id*.txt,
                selected_*.npy}
Outputs       : microct/animal_1/extracted_ossicles/Data
                Calculated/thickness_data_lumped_scaled/<ossicle>.pickle (data tree;
                read by plot_thickness_scaled_small.py)
Environment   : environment-analysis.yml (Python 3.7)
Run           : python thickness_with_dist_lumped_scaled.py
"""
...
for jj in list(range(len(active_loc_list))): #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if os.path.isfile(os.path.join(datadir, 'thickness_data_lumped', file_names[ii][0:-4]+'.pickle')):
        #load pickle file
        with open(os.path.join(datadir, 'thickness_data_lumped', file_names[ii][0:-4]+'.pickle'), 'rb') as f:
            data_whole_list = pickle.load(f)

        #check for nan values in data_whole_list
        for data in data_whole_list:
            if np.isnan(data[2]) or np.isnan(data[1]):
                print('nan value found')
                data_whole_list.remove(data)

        #append data from data_whole_list to list area and dist lists
        for data in data_whole_list:
            whole_dist_list1.append(data[1])
            whole_thick_list1.append(data[2])
...
for jj in list(range(len(active_loc_list))): #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if os.path.isfile(os.path.join(datadir, 'thickness_data_lumped', file_names[ii][0:-4]+'.pickle')):
        ...
        #append data from data_whole_list to list area and dist lists
        for data in data_whole_list:
            whole_dist_list2.append(data[1])
            whole_thick_list2.append(data[2])


#find max and min thickness values
max_thick1 = np.max(np.array(whole_thick_list1))
min_thick1= np.min(np.array(whole_thick_list1))

#find max and min distance values
max_dist1 = np.max(np.array(whole_dist_list1))
min_dist1 = np.min(np.array(whole_dist_list1))

#find max and min thickness values
max_thick2 = np.max(np.array(whole_thick_list2))
min_thick2 = np.min(np.array(whole_thick_list2))

#find max and min distance values
max_dist2 = np.max(np.array(whole_dist_list2))
min_dist2 = np.min(np.array(whole_dist_list2))

max_dist = max(max_dist1, max_dist2)
max_thick = max(max_thick1, max_thick2)

#create bins from range of distances from origin
bin_edges = np.arange(-0.001, max_dist + 0.001,0.002)/max_dist
...
for jj in list(range(len(active_loc_list))): #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if os.path.isfile(os.path.join(datadir, 'thickness_data_lumped', file_names[ii][0:-4]+'.pickle')):
        ...
        #get max and min values for thickness and distance
        max_thick_test = np.max(np.array(data_whole_list)[:,2])
        min_thick_test = np.min(np.array(data_whole_list)[:,2])
        max_dist_test = np.max(np.array(data_whole_list)[:,1])
        min_dist_test = np.min(np.array(data_whole_list)[:,1])

        #append data from data_whole_list to list area and dist lists while scaling them from 0 to 1
        for data in data_whole_list:
            x.append(data[1]/max_dist_test)
            y.append((data[2]-min_thick_test)/(max_thick_test-min_thick_test))#**(1/max_dist_test)) 
        ...
        data_lumped_scaled_list = []
        for aa in range(len(bin_centers)):
            data_lumped_scaled = (bin_centers[aa], bin_means[aa], bin_stdevs[aa])
            data_lumped_scaled_list.append(data_lumped_scaled)

        #save data_lumped_scaled_list as pickle file
        with open(os.path.join(datadir, 'thickness_data_lumped_scaled', file_names[ii][0:-4]+'.pickle'), 'wb') as ff:
            pickle.dump(data_lumped_scaled_list, ff)
