...
"""Medial thickness and path length for the foot-pad graphs (data producer).
...
What it does  : Foot-pad version of thickness_with_dist_lumped.py (active: first
                foot-pad graph, jj = 0).
Inputs        : footpad/graphs/test{1,2}graph_{simple_origin,full}.gpickle (foot-pad
                graphs from 02_image_morphometrics_2d/footpad)
Outputs       : footpad/graphs/thick_with_dist_lump_<graph>.pkl (the readers use
                footpad/graphs/thick_data/, where the files were moved by hand)
Environment   : environment-analysis.yml (Python 3.7)
Run           : python thickness_with_distance_lumped_foot.py
"""
...
for jj in [0]: #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if 1==2:
        ...
        origin = assign_origin(full_G, G)
        #get the position and create a pv sphere mesh at origin
        origin_pos = full_G.nodes[origin]['pos']
        ...
        data_whole_list = get_dist_thickness_origin_to_all(full_G, G)
        #save data_whole_list as a pickle file
        data_file_name = graph_folder + '\\thick_with_dist_lump_' + simple_graph_file_names[jj] + '.pkl'
        with open(data_file_name, 'wb') as f:
            pickle.dump(data_whole_list, f)


        # plot all boundary nodes as spheres
        boundary_nodes = [node for node in G.nodes() if G.nodes[node]['level_bound_topo'] == 1000]
        for node in boundary_nodes:
            node_pos = G.nodes[node]['pos']
