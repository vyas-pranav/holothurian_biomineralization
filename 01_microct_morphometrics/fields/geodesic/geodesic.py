...
"""Geodesic distance maps on an ossicle and on a flat disc.

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Geodesic panels (companion of Fig. S3G); the Fig. 2H path lengths are
                graph path lengths computed in fields/thickness, not by this script
What it does  : Computes exact geodesic distances (pygeodesic) on the aligned surface of
                ossicle_9 from the vertex below the origin and, for comparison, on a
                remeshed flat disc; renders both with the same colour scale (Paired,
                0-0.11 mm), off-screen.
Inputs        : microct/animal_1/extracted_ossicles/aligned/{vtk,
                skeleton/<ossicle>/...}, microct/animal_1/extracted_ossicles/Data
                Calculated/loc_file_id.txt
Outputs       : none (the screenshot 9_dist_disc_back.png is commented out)
Environment   : environment-analysis.yml (Python 3.7)
Run           : python geodesic.py
"""
...
def max_angle_point_below_point(points, reference_point):
    # Find the distance of each point to the reference_point
    distances_to_reference_point = np.linalg.norm(points - reference_point, axis=1)
    ...
    # Find the index of the point with the lowest cosine value (i.e. the point that subtends the least angle on the z-axis)
    index_of_point_with_max_angle = np.argmax(cos_angles)

    # Return the point with the lowest cosine value
    return below_reference_points[index_of_point_with_max_angle]
...
#%%
for ii in [loc_file_id[8]]: #[loc_file_id[4]]: #range(n_files):      print(ii, files[ii])
    ...
    geoalg = geodesic.PyGeodesicAlgorithmExact(ossicle.points(), ossicle.faces())

    origin = np.array([0,0,0])
    # Use source and target point ids
    pt1 = np.argwhere(np.all(ossicle.points() == least_angle_point_below_point(ossicle.points(), start_pt), axis=1)) 
    # pt1 = np.argwhere(np.all(ossicle.points() == closest_point(ossicle.points(), start_pt), axis=1)) 
    # pt2 = np.argwhere(np.all(ossicle.points() == furthest_point_by_x(ossicle.points()), axis=1)) 
    ...
    xy_dist = xy_plane_distance(ossicle.points()[pt1[0][0]],ossicle.points())
    # line = vedo.Line(path).c("k").lw(4)
    # ossicle.cmap("jet", distances, name="GeodesicDistance")
    # vedo.show(ossicle, axes=1)


    center_sph_mesh = vedo.Sphere(ossicle.points()[pt1[0][0]], r=0.002, res=8)
    origin_sph_mesh = vedo.Sphere((0,0,0), r=0.001, res=8)
    start_sph_mesh = vedo.Sphere(start_pt, r=0.003, res=8)


    pv_ossicle = pv.wrap(ossicle.polydata())
    pv_ossicle2 = pv.wrap(ossicle.polydata())
    pv_cl_skel = pv.wrap(cl_skeleton.polydata())
    ...
    pv_ossicle.point_data.set_scalars(distances, 'geodesic distances from center')
    pv_ossicle2.point_data.set_scalars(xy_dist, 'xy plane projected distance from center')
    contours = pv_ossicle.contour(isosurfaces=100)
    contours2 = pv_ossicle2.contour(isosurfaces=100)



    cmap = mpl.cm.get_cmap('Paired')
    # cmap = mpl.cm.get_cmap('rainbow', 20) #('tab20')
    # rgba = cmap(1/22 + 1/11)
    # osc_color = pv.Color([rgba[0], rgba[1], rgba[2] ])
    pl.add_mesh(pv_ossicle, cmap = cmap, opacity = 1, show_scalar_bar = False, clim = [0,0.11])#osc_color)#, specular=1.0, specular_power=10)

    # pv_ossicle.set_active_scalars('geodesic distances from center')
    # pl.add_mesh(pv_ossicle2, opacity = 0.5, clim = [0,0.13])#osc_color)#, specular=1.0, specular_power=10)

    pl.add_mesh(pv_center, color='red')#osc_color)#, specular=1.0, specular_power=10)
    # pl.add_mesh(pv_cl_skel, color='red')#osc_color)#, specular=1.0, specular_power=10)
    # pl.add_mesh_isovalue(pv_ossicle, scalars = 'geodesic distances from center')  #add single isovalue line with slider
    # pl.add_mesh(contours, cmap = cmap, line_width=2, clim = [0, 0.13], show_scalar_bar = False)   #25
    # pl.add_mesh(contours2, cmap = cmap, line_width=10, clim = [0, 0.13],  show_scalar_bar = False)  #10

    # pl.add_mesh(pv_origin, color = 'black')
    # pl.add_mesh(pv_start, color = 'blue')

    # for jj in range(10):
    # pv_disc = pv.Disc(center=(ossicle.points()[pt1[0][0]][0], ossicle.points()[pt1[0][0]][1], -0.01), inner=0.001, outer=0.05, normal=(0, 0, 1), r_res=100, c_res=60)
    # pl.add_mesh(pv_disc, color = 'grey', show_edges=True, line_width=1, opacity = 0.5)
    # pl.add_mesh(pv_origin, color='orange')#osc_color)#, specular=1.0, specular_power=10)

# center_coord = ossicle.points()[pt1[0][0]] + np.array([0,0,0.02])
# pv_disc = pv.Disc(center=center_coord, inner=0, outer=0.1, normal=(0, 0, 1), r_res=50, c_res=50)

center_coord = ossicle.points()[pt1[0][0]] + np.array([0,0,0.019])
pv_disc = pv.Disc(center=center_coord, inner=0, outer=0.05, normal=(0, 0, 1), r_res=50, c_res=50)

# pv_cyl = pv.PolyData(vedo_cyl.points(), vedo_cyl.faces())
pv_cyl = remesh_func(pv_disc,2,10000) 
# pv_cyl = remesh_func(pv.Cylinder(center=center_coord, direction=(0, 0.0, 1), radius=0.05, height=0.002, resolution=100, capping=True), 2,10000)
vedo_cyl = Mesh(pv_cyl)  #vedo.Cylinder(pos=center_coord, r=0.05, height=0.002, axis=(0, 0, 1), cap=True, res=24, c='teal3',alpha=1)#

geoalg1 = geodesic.PyGeodesicAlgorithmExact(vedo_cyl.points(), vedo_cyl.faces())
pt2 = np.argwhere(np.all(vedo_cyl.points() == least_angle_point_below_point(vedo_cyl.points(), center_coord), axis=1)) 

distances1, best_source = geoalg1.geodesicDistances(pt2[0], None) # any of the two
pv_cyl.point_data.set_scalars(distances1, 'geodesic distances from center')
pv_sph_cyl = pv.Sphere(radius = 0.002, center = pv_cyl.points[pt2[0][0]])

# pl.add_mesh(pv_sph_cyl, cmap = cmap, line_width = 1, style = 'surface', show_edges = True, color = 'red')
pl.add_mesh(pv_cyl, cmap = cmap, line_width = 1, style = 'surface', show_edges = False, opacity = 1, show_scalar_bar = False, clim = [0,0.11])

# show the meshes
pl.add_scalar_bar(title='Distance (in mm)', mapper = None, n_labels=5, italic=False, bold=False, title_font_size=None, label_font_size=None, color=None, font_family=None, shadow=False, width=None, height=None, position_x=None, position_y=None, vertical=None, interactive=None, fmt=None, use_opacity=True, outline=False, nan_annotation=False, below_label=None, above_label=None, background_color=None, n_colors=None, fill=False, render=False, theme=None)
pl.enable_parallel_projection()

pl.add_axes()
pl.camera.roll = 90.0
# pl.screenshot('9_dist_disc_back.png', window_size=(10000,10000), transparent_background= True)
# pl.show()
# 
