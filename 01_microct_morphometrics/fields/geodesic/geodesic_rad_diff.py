...
"""Geodesic minus in-plane radial distance on an ossicle.

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Fig. S3G
What it does  : Computes exact geodesic distances (pygeodesic) on ossicle_9 from the
                vertex below the origin, subtracts the xy-projected (radial) distance
                from the same vertex and renders the difference (plasma, 0-0.01 mm),
                off-screen.
...
Outputs       : 9_dist_diff.png (10000 x 10000 px) in the working directory
Environment   : environment-analysis.yml (Python 3.7)
Run           : python geodesic_rad_diff.py
"""
...
for ii in [loc_file_id[8]]: #[loc_file_id[4]]: #range(n_files):      print(ii, files[ii])
    ...
    dist_diff = distances - xy_dist

    print(np.max(dist_diff))
    ...
    pv_ossicle2.point_data.set_scalars(dist_diff, 'distance difference from radial distance')
    ...
    pl.add_mesh(pv_ossicle2, cmap = cmap, opacity = 1, show_scalar_bar = False, clim = [0, 0.01])#osc_color)#, specular=1.0, specular_power=10)
...
pl.camera.roll = 90.0
pl.screenshot('9_dist_diff.png', window_size=(10000,10000), transparent_background= True)
# pl.show()
# 
