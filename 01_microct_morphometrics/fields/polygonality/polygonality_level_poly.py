...
"""Hole depth and number of sides (data producer).

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Hole-polygonality panel (Fig. S3H)
What it does  : For each pillared table (selected_table) takes the ordered minimum cycle
                basis of origin_clean_simple_base_G and pickles, per hole, the lowest
                topological depth of its nodes and its number of sides. As in the
                original workflow, the other-plate pickles were produced by editing
...
Outputs       : microct/animal_1/extracted_ossicles/Data
                Calculated/level_poly_data/<ossicle>.pickle (data tree; read by
                level_poly_plot.py)
Environment   : environment-analysis.yml (Python 3.7)
Run           : python polygonality_level_poly.py
"""
...
def load_graphs(ii):
    ...
    clean_full_base_G = nx.read_gpickle(os.path.join(skel_paths[ii],"clean_full_G.gpickle"))
    ...
    return clean_full_G, clean_simple_G, origin_clean_simple_G, clean_full_base_G, clean_simple_base_G, origin_clean_simple_base_G, area_mid_G, area_mid_base_G
...
#get hole polygonality along with the centroid of each hole
def get_hole_polygonality(min_cycle_list, G):
    polygonality_list = []
    centroid_list = []
    for cycle in min_cycle_list:
        polygonality_list.append(len(cycle))
    ...
    return polygonality_list, centroid_list
...
def get_level_polygonality_list(min_cycle_basis, G):
    polygonality, centroid_list = get_hole_polygonality(min_cycle_basis, G)

    min_level_list = []
    for cycle in min_cycle_basis:
        #get the least level_bound_topo value amongst all the nodes in the cycle
        level_list = []
        for node in cycle:
            level_list.append(G.nodes[node]['level_topo'])
        min_level = min(level_list)    
        min_level_list.append(min_level)

    #create a single list with entries as tuple(distance, polygonality)
    level_poly_list = []
    ctr = 0
    for min_level in min_level_list:
        level_poly_list.append((min_level, polygonality[ctr]))
        ctr+=1

    print(level_poly_list)

    return level_poly_list
...
for jj in list(range(len(active_loc_list))): #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if len(skeleton.lines())!=0 and len(cl_skeleton.lines())==0:
        ...
        clean_full_G, clean_simple_G, origin_clean_simple_G, clean_full_base_G, clean_simple_base_G, origin_clean_simple_base_G, area_mid_G, area_mid_base_G = load_graphs(ii)
        G = area_mid_base_G
        G1 = origin_clean_simple_base_G
        ...
        min_cycle_basis_G1 = get_ordered_min_cycle_basis(G1)
        # poly_surf = get_hole_surf_list(min_cycle_basis_G1, G1)

        #get the distance from origin and thickness value for all nodes
        data_whole_list = get_level_polygonality_list(min_cycle_basis_G1, origin_clean_simple_base_G)

        #save data_whole_list as a pickle file
        with open(os.path.join(datadir, 'level_poly_data', file_names[ii][0:-4]+'.pickle'), 'wb') as f:
        ...
        #     pv_hole.point_data['poly'] = polygonality[ctr] * np.ones([pv_hole.n_points,1])
        #     pv_hole.set_active_scalars('poly')            
        ...
        # ctr = 0
        # for surf in hole_mesh_list:
        #     pl.add_mesh(pv_hole_list[ctr], cmap = cmap, clim = [2,10], show_scalar_bar= False)    #(pv.wrap(surf.polydata()))
        #     ctr += 1
    # # pl.add_mesh()
                # plot all the poly_surf
        # for surf in poly_surf:
        #     pv_surf = surf.copy()
        #     pv_surf.point_data['poly'] = polygonality[ctr] * np.ones([pv_surf.n_points,1])
        #     pv_surf.set_active_scalars('poly')
        #     pl.add_mesh(pv_surf, cmap = cmap, clim = [2, 10], show_scalar_bar= False)    #(pv.wrap(surf.polydata()))
        #     ctr+=1
