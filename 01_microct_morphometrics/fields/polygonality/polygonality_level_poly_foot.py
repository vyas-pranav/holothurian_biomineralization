...
"""Hole depth and number of sides of the foot-pad graphs (data producer).
...
What it does  : Foot-pad version of polygonality_level_poly.py for both foot-pad graphs.
Inputs        : footpad/graphs/test{1,2}graph_{simple_origin,full}.gpickle (foot-pad
                graphs from 02_image_morphometrics_2d/footpad)
Outputs       : footpad/graphs/poly_by_level_<graph>.pkl (the plotter reads them from
                footpad/graphs/poly_data/, where they were moved by hand)
Environment   : environment-analysis.yml (Python 3.7)
Run           : python polygonality_level_poly_foot.py
"""
...
for jj in [0,1]: #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if 1==2:
        ...
        data_whole_list = get_level_polygonality_list(min_cycle_basis_G1, G1)

        #save data_whole_list as a pickle file
        data_file_name = graph_folder + '\\poly_by_level_' + simple_graph_file_names[jj] + '.pkl'
