...
"""Holes of an ossicle coloured by number of sides.
...
What it does  : Cuts the base surface of ossicle_9 (loc_file_id[8]) into hole patches
                and colours them by the number of sides of the corresponding cycle of
                clean_simple_base_G (plasma, 2-12), off-screen.
...
Outputs       : 9_poly.png (10000 x 10000 px) in the working directory
Environment   : environment-analysis.yml (Python 3.7)
Run           : python polygonality_fields.py
"""
...
def get_hole_polygonality(min_cycle_list):
    polygonality_list = []
    centroid_list = []
...
active_loc_list = loc_file_id

for jj in [8]:  # list(range(0,10))+list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])

    ii = active_loc_list[jj]
    ossicle, voxel_vol, skeleton, cl_skeleton = load_data(ii)
    ...
    if len(skeleton.lines())!=0 and len(cl_skeleton.lines())==0:
        ...
        min_cycle_basis_G1 = get_min_cycle_basis(G1)
        polygonality, centroid_list = get_hole_polygonality(min_cycle_basis_G1)
        poly_surf = get_hole_surf_list(min_cycle_basis_G1, G1)


        # for ii in range(len(hole_center_list)):
        ...
        ctr = 0
        for surf in hole_mesh_list:
            pl.add_mesh(pv_hole_list[ctr], cmap = cmap, clim = [2,12], show_scalar_bar= False)    #(pv.wrap(surf.polydata()))
            ctr += 1
    # # pl.add_mesh()
