...
"""Foot-pad graphs with edges coloured by length.
...
What it does  : Renders both foot-pad graphs as edge cylinders coloured by edge length
                (plasma, 0-0.03 mm), the second pad shifted beside the first,
                off-screen.
Inputs        : footpad/graphs/test{1,2}graph_{simple_origin,full}.gpickle (foot-pad
                graphs from 02_image_morphometrics_2d/footpad)
Outputs       : foot_pad_coupled_1.png (10000 x 10000 px) in the working directory
Environment   : environment-analysis.yml (Python 3.7)
Run           : python length_field_foot.py
"""
...
for jj in [0,1]: #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if 1==2:
        ...
        if jj is 1:
            #change x and y coordinates in the graph 
            for node in G.nodes():
                pos = G.nodes[node]['pos']
                G.nodes[node]['pos'] = [pos[0] + 0.11, pos[1] - 0.14, pos[2]]
        ...
        #     ctr+=1
        cyl_list, length_list = nx_to_pv_cylinders(G, 0.002, 40)
...
pl.camera.roll = 90.0
pl.screenshot('foot_pad_coupled_1.png', window_size=(10000,10000), transparent_background= True)
pl.show()
