...
"""Edge length and edge depth of the foot-pad graphs (data producer).
...
What it does  : Foot-pad version of length_by_level.py: assigns the origin of each
                foot-pad graph and pickles (edge depth, edge length) for both pads.
Inputs        : footpad/graphs/test{1,2}graph_{simple_origin,full}.gpickle (foot-pad
                graphs from 02_image_morphometrics_2d/footpad)
Outputs       : footpad/graphs/len_by_level_<graph>.pkl (the plotter reads them from
                footpad/graphs/len_data/, where they were moved by hand)
Environment   : environment-analysis.yml (Python 3.7)
Run           : python length_by_level_foot.py
"""
...
for jj in [0,1]: #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if 1==2:
        ...
        data_whole_list = get_edge_level_length(G)

        #save data_whole_list as a pickle file
        data_file_name = graph_folder + '\\len_by_level_' + simple_graph_file_names[jj] + '.pkl'
