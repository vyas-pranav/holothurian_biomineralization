...
"""Edge length vs edge depth for tables, other plates and foot pads.
...
What it does  : Pools (edge depth, edge length) of the pillared tables, other plates and
                foot pads and plots binned mean +/- SD with the raw points.
Inputs        : microct/animal_1/extracted_ossicles/Data
                Calculated/edge_level_length/*.pickle,
                footpad/graphs/len_data/len_by_level_*.pkl,
...
Outputs       : OUT_ROOT/01_microct_morphometrics/edge_length_vs_level3.{png,svg} (the
                folder must exist)
Environment   : environment-analysis.yml (Python 3.7)
Run           : python plot_length_by_level.py
"""
...
for jj in list(range(len(active_loc_list))): #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if os.path.isfile(os.path.join(datadir, 'edge_level_length', file_names[ii][0:-4]+'.pickle')):
        print('file exists')
        #load pickle file
        ...
        for data in data_whole_list:
            whole_level_list1.append(data[2])
            whole_length_list1.append(data[3])
...
for jj in list(range(len(active_loc_list))): #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if os.path.isfile(os.path.join(datadir, 'edge_level_length', file_names[ii][0:-4]+'.pickle')):
        ...
        #append data from data_whole_list to list area and dist lists
        for data in data_whole_list:
            whole_level_list2.append(data[2])
            whole_length_list2.append(data[3])
...
for jj in [0,1]: #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if 1==2:
        ...
        data_file_name = graph_folder + '\\len_data\\len_by_level_' + simple_graph_file_names[jj] + '.pkl'
        ...
        for data in data_whole_list:
            if (np.isnan(data[0]) or np.isnan(data[1])):
                ...
                whole_level_list3.append(data[2])
                whole_length_list3.append(data[3])
...
#save figure
length_folder = str(OUT_ROOT / "01_microct_morphometrics")
os.makedirs(length_folder, exist_ok=True)
plt.savefig(os.path.join(length_folder,'edge_length_vs_level3.png'), dpi = 300, bbox_inches = 'tight')
plt.savefig(os.path.join(length_folder,'edge_length_vs_level3.svg'), dpi = 300, bbox_inches = 'tight')
