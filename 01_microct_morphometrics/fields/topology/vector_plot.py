...
"""Growth-direction arrows on an ossicle coloured by topological depth.
...
What it does  : Draws ossicle_207 (loc_file_id[206]) as a translucent grey mesh with
                arrows along the edges of origin_clean_simple_base_G pointing from lower
                to higher topological depth and node spheres coloured by topological
                depth (plasma, 0-12), off-screen.
...
Outputs       : 207_front_vector.png (10000 x 10000 px) in the working directory
Environment   : environment-analysis.yml (Python 3.7)
Run           : python vector_plot.py
"""
...
for jj in list(range(206,207)):      #print(ii, files[ii])
    do_screenshot = 'True'
    if do_screenshot:
