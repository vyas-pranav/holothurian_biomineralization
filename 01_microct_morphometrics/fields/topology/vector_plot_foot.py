...
"""Growth-direction arrows on a foot-pad graph.
...
What it does  : Arrows from lower to higher topological depth and depth-coloured node
                spheres (plasma, 0-12) for the second foot-pad graph (jj = 1).
Inputs        : footpad/graphs/test{1,2}graph_{simple_origin,full}.gpickle (foot-pad
                graphs from 02_image_morphometrics_2d/footpad)
Outputs       : interactive PyVista window (do_screenshot = False)
Environment   : environment-analysis.yml (Python 3.7)
Run           : python vector_plot_foot.py
"""
...
for jj in [1]: #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if 1==2:
        ...
        # img_folder = str(DATA_ROOT / "footpad" / "masks")
        # img = pv.read(os.path.join(img_folder, 'test'+str(jj+1)+'_mask.bmp'))   
        # #scale image size to match the mesh
        # scale_factor = scale_factor_list[jj]
        # pl.add_mesh(img, opacity = 0.5, show_scalar_bar=False, cmap = 'gray')
        #add image as background image
        # pl.add_background_image(os.path.join(img_folder, 'test'+str(jj+1)+'_mask.bmp'))

        # #scale all dimensions of the graph by the scale factor
        # for node in G.nodes():
        #     G.nodes[node]['pos'] = G.nodes[node]['pos']/scale_factor
    ...
    if do_screenshot:
        filename_png = str(jj+1) + '_front_vector.png'
