...
"""Topological-depth tree of ossicle base graphs.
...
What it does  : For ossicles loc_file_id[8] and [9] (ossicle_9, ossicle_10) draws the
                base graph origin_clean_simple_base_G as a flat tree: nodes at (x,
                -topological depth), arrows from lower to higher depth (boundary edges,
                level >= 1000, skipped), node spheres coloured by topological depth
                (plasma, 0-12), parallel projection, off-screen.
...
Outputs       : <n>_topo_plot.png (n = 9, 10; 10000 x 10000 px) in the working directory
Environment   : environment-analysis.yml (Python 3.7)
Run           : python topology_plot.py
"""
...
for jj in range(8,10): #range(n_files):      print(ii, files[ii])
