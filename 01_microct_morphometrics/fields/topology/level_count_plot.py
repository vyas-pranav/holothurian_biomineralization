...
"""Number of nodes per topological depth.

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Topology panels (Fig. 2C, 2E; Fig. S2C-D); this script:
                node_count_vs_level3.svg
What it does  : Loads the per-depth node counts of the pillared tables and other plates
                (level_count_data/whole_{level,count}_list{1,2}.pickle) and of the two
                foot pads (footpad/graphs/topo_data/level_count_*.pkl) and plots binned
                mean +/- SD with the raw points for the four series. The commented
                blocks record how these pickles were produced from
                origin_clean_simple_base_G and the savefig of
                node_count_vs_level3.svg/.png.
Inputs        : microct/animal_1/extracted_ossicles/Data
                Calculated/{level_count_data/whole_*_list{1,2}.pickle, loc_file_id*.txt,
                selected_*.npy}; footpad/graphs/topo_data/level_count_*.pkl;
                footpad/graphs/test{1,2}graph_{simple_origin,full}.gpickle (foot-pad
                graphs from 02_image_morphometrics_2d/footpad)
Outputs       : matplotlib window (savefig of node_count_vs_level3.svg/.png is commented
                out)
Environment   : environment-analysis.yml (Python 3.7)
Run           : python level_count_plot.py
"""
...
#         # #count the number of nodes belonging to each level
#         # level_count = {}
...
#         # with open(os.path.join(datadir, 'level_count_data', file_names[ii][0:-4]+'.pickle'), 'wb') as f:
#         #     pickle.dump(level_count, f)

#         with open(os.path.join(datadir, 'level_count_data', file_names[ii][0:-4]+'.pickle'), 'rb') as f:
#             level_count = pickle.load(f)

#         level_list = []
#         count_list = []
#         for level in level_count:
#             level_list.append(level)
#             count_list.append(level_count[level])

#         #add the level and count list to the whole lists
#         for level in level_list:
#             whole_level_list1.append(level)

#         for count in count_list:
#             whole_count_list1.append(count)

# #save the whole lists
# with open(os.path.join(datadir, 'level_count_data', 'whole_level_list1.pickle'), 'wb') as f:
#     pickle.dump(whole_level_list1, f)

# with open(os.path.join(datadir, 'level_count_data', 'whole_count_list1.pickle'), 'wb') as f:
#     pickle.dump(whole_count_list1, f)
...
#         #add the level and count list to the whole lists
#         for level in level_list:
#             whole_level_list2.append(level)

#         for count in count_list:
#             whole_count_list2.append(count)


# #save the whole lists
# with open(os.path.join(datadir, 'level_count_data', 'whole_level_list2.pickle'), 'wb') as f:
#     pickle.dump(whole_level_list2, f)

# with open(os.path.join(datadir, 'level_count_data', 'whole_count_list2.pickle'), 'wb') as f:
#     pickle.dump(whole_count_list2, f)
...
for jj in [0]: #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if 1==2:
        ...
        data_file_name = graph_folder + '\\topo_data\\level_count_' + simple_graph_file_names[jj] + '.pkl'
        ...
        #add the level and count list to the whole lists
        for level in level_list:
            whole_level_list3.append(level)

        for count in count_list:
            whole_count_list3.append(count)
...
for jj in [1]: #list(range(0,10))+ list(range(11,len(active_loc_list))): #range(n_files):      print(ii, files[ii])
    ...
    if 1==2:
        ...
        #add the level and count list to the whole lists
        for level in level_list:
            whole_level_list4.append(level)

        for count in count_list:
            whole_count_list4.append(count)



# #save the whole lists
# with open(os.path.join(datadir, 'level_count_data', 'whole_level_list2.pickle'), 'wb') as f:
#     pickle.dump(whole_level_list3, f)

# with open(os.path.join(datadir, 'level_count_data', 'whole_count_list2.pickle'), 'wb') as f:
#     pickle.dump(whole_count_list3, f)



#load the whole lists
with open(os.path.join(datadir, 'level_count_data', 'whole_level_list1.pickle'), 'rb') as f:
    whole_level_list1 = pickle.load(f)

with open(os.path.join(datadir, 'level_count_data', 'whole_count_list1.pickle'), 'rb') as f:
    whole_count_list1 = pickle.load(f)

with open(os.path.join(datadir, 'level_count_data', 'whole_level_list2.pickle'), 'rb') as f:
    whole_level_list2 = pickle.load(f)

with open(os.path.join(datadir, 'level_count_data', 'whole_count_list2.pickle'), 'rb') as f:
    whole_count_list2 = pickle.load(f)
...
min_level = min(whole_level_list1)
max_level = max(whole_level_list1)
...
#find min and max values of distances from origin present in the second entry of all the tuples in data_whole_list
min_level = min(whole_level_list2)
max_level = max(whole_level_list2)
...
#find min and max values of distances from origin present in the second entry of all the tuples in data_whole_list
min_level = min(whole_level_list3)
max_level = max(whole_level_list3)
...
#also mark points on the mean curve
plt.scatter(x, y, marker = 'o', color = color2, alpha = 0.1, s = 25, zorder = 2, linewidth = 0)
plt.scatter(bin_centers, bin_means, marker = 'o', color = color2, s = 100, zorder = 3, alpha = 0.5, label = 'foot pad 1', linewidth = 0)
...
#find min and max values of distances from origin present in the second entry of all the tuples in data_whole_list
min_level = min(whole_level_list4)
max_level = max(whole_level_list4)
...
#also mark points on the mean curve
plt.scatter(x, y, marker = 'o', color = color2, alpha = 0.1, s = 25, zorder = 2, linewidth = 0)
plt.scatter(bin_centers, bin_means, marker = 'o', color = color2, s = 100, zorder = 3, alpha = 0.5, label = 'foot pad 2', linewidth = 0)
...
# #save figure
# plt.savefig(os.path.join('node_count_vs_level3.svg'), dpi = 300, bbox_inches = 'tight')
# plt.savefig(os.path.join('node_count_vs_level3.png'), dpi = 300, bbox_inches = 'tight')
