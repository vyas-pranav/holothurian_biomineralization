...
"""Ossicles at their positions in the animal, coloured by average mean curvature.

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Whole-animal render (Fig. 1C / S3A per the release plan; not confirmed
                from the code)
What it does  : Loads unaligned ossicle meshes (scan coordinates) and colours each by an
                entry of avg_mean_curvature50.npy (plasma, -20..20). The active loop
                renders only the first 10 pillared tables (loc_file_id_table[0:10]) in
                an interactive window; the commented screenshot is ani_volume_map.png.
...
max_val = 0
min_val = 0
for jj in range(10): #[loc_file_id[4]]: #range(n_files):      print(ii, files[ii])

    ii = loc_file_id_table[jj]
    ...
    if os.path.isfile(os.path.join(curv_dir, 'mean', f_name)):
        ...
        pl.add_mesh(pv_ossicle, cmap = cmap, clim = [-20,20], opacity = 1, diffuse = 0.9, smooth_shading = True, show_scalar_bar= False)#, clim = [0,0.13])#osc_color)#, specular=1.0, specular_power=10)
