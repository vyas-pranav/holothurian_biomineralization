...
"""Top view of a pillared table for schematic figures.

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Schematic base image (Fig. S17 per the release plan; not confirmed from
                the code)
What it does  : Renders the aligned mesh of ossicle_25 (loc_file_id[24]) in light grey
                from above with x-axis bounds as a scale, off-screen; commented blocks
                colour split nodes by bifurcation angle.
...
Outputs       : 25_top_sc.png (10000 x 10000 px) in the working directory
Environment   : environment-analysis.yml (Python 3.7)
Run           : python phase_cartoon.py
"""
...
for jj in range(24,25): #range(n_files):      print(ii, files[ii])
    ...
    if len(skeleton.lines())!=0 and len(cl_skeleton.lines())!=0:
        ...
        pl.add_mesh(pv_ossicle, color = 'lightgrey', opacity = 1, diffuse = 0.9, smooth_shading = True, show_scalar_bar= True)#, clim = [0,0.13])#osc_color)#, specular=1.0, specular_power=10)
        # pl.add_mesh(pv_skeleton, cmap= cmap)
        # pl.add_mesh(pv_rem_recon, color = 'yellow', opacity =0.5,style = 'wireframe')    
        # pl.add_mesh(pv_graph, color = 'black', render_lines_as_tubes=True, render_points_as_spheres = True, style='wireframe', line_width=40, point_size = 40, show_scalar_bar=False) #line_width = 50
        ...
        #     pos = G.nodes[triplet[0]]['pos']
        #     pos_list.append(pos)
        #     angle_list.append(angle)

        # for pos, angle in zip(pos_list, angle_list):
        #     sph_mesh = pv.Sphere(radius = 0.004, center = pos)
        #     sph_mesh.point_data['angle'] = angle*np.ones([sph_mesh.n_points,1])
        #     pl.add_mesh(sph_mesh, cmap = cmap, clim = [60, 120], show_scalar_bar= False)
    ...
    # # pl.add_mesh()
        ...
        # colors  = vedo.color_map(level_list, "plasma_r", vmin = 0, vmax = 15 )
        # node_meshes = vedo.Spheres(pos_list, c = colors, r = 0.003)
...
# show the meshes
pl.show_bounds(show_zaxis = False, show_yaxis = False)
...
pl.camera.roll = 270.0
pl.screenshot('25_top_sc.png', window_size=(10000,10000), transparent_background= True)
# pl.show()
