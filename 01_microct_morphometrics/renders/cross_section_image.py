...
"""Cross-section view of an ossicle.

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Cross-section render (Fig. 2A middle per the release plan; not confirmed
                from the code)
What it does  : Clips the aligned mesh of ossicle_80 with a y-normal plane through its
                centre and shows the cut ossicle in an interactive window (the mesh
                carries no medial-thickness data, so the colour map and scalar bar are
                not data-driven).
Inputs        : microct/animal_1/extracted_ossicles/aligned/{vtk,
                ...
                avg_mean_curvature.npy}
Outputs       : interactive PyVista window
Environment   : environment-analysis.yml (Python 3.7)
Run           : python cross_section_image.py
"""

#get cross sectional image of an ossicle and an entire animal
...
pv.global_theme.camera = {'position': [-1,1,0],'viewup': [0, 0, -1]}  #[-1, -0.3, 2.2]
...
max_val = 0
min_val = 0
for jj in [79]: #range(n_files):      print(ii, files[ii])
    ...
    clipped = pv_ossicle.clip('y', invert=False)
    # pl.add_mesh(pv_ossicle, cmap = cmap, clim = [0,0.01], opacity = 1, diffuse = 0.9, smooth_shading = True, show_scalar_bar= False)#, clim = [0,0.13])#osc_color)#, specular=1.0, specular_power=10)
    pl.add_mesh(clipped, cmap = cmap, clim = [0,0.01], opacity = 1, diffuse = 0.9, smooth_shading = True, show_scalar_bar= False)#, clim = [0,0.13])#osc_color)#, specular=1.0, specular_power=10)
...
pl.camera.roll = 90.0
# pl.screenshot('206_table_area.png', window_size=(10000,10000), transparent_background= True)
pl.show()
# 
