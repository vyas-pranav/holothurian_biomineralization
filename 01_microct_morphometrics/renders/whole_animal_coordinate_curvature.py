...
"""Whole-animal map of ossicle base curvature.

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Fig. 2A (right)
What it does  : Loads all 213 unaligned ossicle meshes (scan coordinates) and colours
                each by -|average mean curvature| of its base surface (plasma, -30..0
                1/mm), off-screen.
Inputs        : microct/animal_1/extracted_ossicles/{vtk, skeleton/<ossicle>/...},
                microct/animal_1/extracted_ossicles/aligned/curvature/mean/*_mean_curv50.npy
                (existence check), microct/animal_1/extracted_ossicles/Data
                Calculated/{loc_file_id.txt, loc_file_id_table.txt, centroids.npy,
                centroid_sorted_indices.npy, avg_mean_curvature50.npy}
Outputs       : ani_curv_map_scale.png (10000 x 10000 px) in the working directory
Environment   : environment-analysis.yml (Python 3.7)
Run           : python whole_animal_coordinate_curvature.py
"""
...
with open(os.path.join(datadir, 'centroid_sorted_indices.npy'), 'rb') as f:
    sort_ind_cent = np.load(f)

with open(os.path.join(datadir, 'avg_mean_curvature50.npy'), 'rb') as f:
    data_list = np.load(f)


data_list = -1*np.abs(data_list)
...
max_val = 0
min_val = 0
for jj in range(213): #[loc_file_id[4]]: #range(n_files):      print(ii, files[ii])
    ...
    if os.path.isfile(os.path.join(curv_dir, 'mean', f_name)):
        ...
        pl.add_mesh(pv_ossicle, cmap = cmap, clim = [-30,0], opacity = 1, diffuse = 0.9, smooth_shading = True, show_scalar_bar= False)#, clim = [0,0.13])#osc_color)#, specular=1.0, specular_power=10)
