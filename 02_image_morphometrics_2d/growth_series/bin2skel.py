...
"""Time-lapse ossicle frames: binary mask to skeleton to lattice graphs.

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Fig. 3H (graphs used for volume/area), Fig. 3J; Fig. S5A (overlays)
What it does  : For each of the five time-lapse frames it cleans the binary mask, takes
                the medial-axis skeleton and builds networkx graphs (sknw, full pixel graph
                and simple graph with medial thickness, simple graph with an origin node at
                the thickest edge). Scale: 20 µm = 43 px.
Inputs        : DATA_ROOT/growth_series/images/*.png (the five frames only) and
                DATA_ROOT/growth_series/masks/*.png (binary masks, same order)
Outputs       : DATA_ROOT/growth_series/graphs/<name>graph_{sknw,full,simple,simple_origin}.gpickle
                and <name>_graph.png (read by sa_vol_calc.py);
                OUT_ROOT/02_image_morphometrics_2d/growth_series/labels/ and skeletons/ (overlays)
Environment   : environment-analysis.yml (Python 3.7)
Run           : python bin2skel.py
"""
...
# output folders of this script (they already existed in the original working folder)
(OUT_ROOT / "02_image_morphometrics_2d" / "growth_series" / "labels").mkdir(parents=True, exist_ok=True)
(OUT_ROOT / "02_image_morphometrics_2d" / "growth_series" / "skeletons").mkdir(parents=True, exist_ok=True)
# ----------------------------------------------------------------------------
...
img_folder = str(DATA_ROOT / "growth_series" / "images")
mask_folder = str(DATA_ROOT / "growth_series" / "masks")
skeleton_folder = str(OUT_ROOT / "02_image_morphometrics_2d" / "growth_series" / "skeletons")
label_folder = str(OUT_ROOT / "02_image_morphometrics_2d" / "growth_series" / "labels")
graph_folder = str(DATA_ROOT / "growth_series" / "graphs")
...
scale_factor =  20/43*0.001       #mm/pixel     
...
for ii in range(len(images)):
    ...
    binary_image = mask #cv2.imread('/image/15-427-1- ps - binary inv.png',0)
