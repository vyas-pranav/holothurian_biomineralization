...
"""Sweep of the number of initial seed branches (2D branching model).

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Figs. 6D-G (2, 4, 5 and 7 seed branches); Fig. S20A (frames of the
                4-branch run; steps 11, 31, ..., 111 are frames ii = 10, 30, ..., 110)
What it does  : For each initial branch count, runs the Ossicle model (ossicle_class3.py)
                with the SI Table 4 parameters and the logarithmic timing rule
                rep_interval = max(tau_min, tau_max * ln(c) / kappa) (SI Eq. 48,
                kappa = 'morphogen_rep_scale' = 4) until no tip is active (at most
                'tsteps' steps). As found, the loop is range(4, 5) (4 branches only) with
                a frame saved at every step (save_plots_every_n = 1); widen the range for
                Figs. 6D-G (earlier versions of this sweep used range(1, 11)).
Inputs        : none
Outputs       : OUT_ROOT/06_branching_lattice_model_2d/Network simulations/
                seed_edge_sweep_study/<YYYYmmdd_HHMMSS>/initial_branch_count_<NN>/:
...
Run           : python seed_edge_sweep3.py   (ossicle_class3.py must be in this folder)
"""
...
#----------------------------------------------------------
# Wrapper function for a single simulation run with given initial branch count.
def run_simulation(initial_branch_count, sim_params, base_run_folder, save_plots_every_n=2, tsteps=None):
    """
    Run one simulation with a specified initial branch count.
    
    Parameters:
        initial_branch_count (int): Number of initial branches.
    ...
    run_folder = os.path.join(base_run_folder, f"initial_branch_count_{initial_branch_count:02d}")
    os.makedirs(run_folder, exist_ok=True)
    plots_folder = os.path.join(run_folder, "plots")
    os.makedirs(plots_folder, exist_ok=True)
    
    # Update simulation parameters for this run.
    sim_params['initial_branch_count'] = initial_branch_count
    ...
    for ii in range(tsteps):
        print(f"[Initial Branch Count = {initial_branch_count}] Time step: {ii}")
    ...
    plt.close(fig)
    print(f"Simulation with Initial Branch Count = {initial_branch_count} complete. Data saved in: {run_folder}")

#----------------------------------------------------------
# Main function to perform the parameter sweep over initial branch count.
def main():
    ...
    base_folder = str(OUT_ROOT / "06_branching_lattice_model_2d" / "Network simulations" / "seed_edge_sweep_study")
    ...
    save_plots_every_n = 1  # Save a plot every 5 time steps.

    # Sweep initial branch count from 4 to 5.
    for branch_count in range(4, 5):
        # Use a fresh copy of the constant parameters for each run.
        sim_params = constant_sim_params.copy()
        run_simulation(branch_count, sim_params, study_folder, save_plots_every_n)
