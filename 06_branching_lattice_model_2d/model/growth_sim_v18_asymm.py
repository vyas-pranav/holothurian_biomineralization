...
"""2D branching model with moving cells (asymmetric, front-plate-like ossicle).

Code release for Vyas et al., "Cellular construction of topologically complex
biomineral lattices in holothurians".

Figure panels : Fig. 6K
What it does  : The model of growth_sim_v18_main.py (same growth and timing rules, see
                Model notes) with three changes: the four cells start at the origin, the
                four seed branches are rotated by pi/4, and the cells move at every step
                (Ossicle.modify_topology): three at 0.3 x (1, 0.75, 0.5) um/step in +x
                and one at 0.3 x 0.5 um/step in -y, which makes the morphogen field and
                the lattice asymmetric. The view is fixed to x in [-40, 80] um,
                y in [-50, 40] um.
                Note: 'morphogen_decay_rate' (k) is 0.010 /h in this file; the Fig. 6K
                legend lists k = 0.02 /h.
...
Outputs       : a live figure; with save_data = True also parameters.txt, frames, final
                PNGs and final_oss_graph(_pruned).gpickle (standard pickles; both hold
                the same, unpruned graph) in OUT_ROOT/06_branching_lattice_model_2d/
                Network simulations/old tests/unique_morphologies_test/<timestamp>/.
                The base folder is created even when saving is off (as in the original).
Environment   : environment-models.yml (Python 3.13); the paper runs used
                environment-analysis.yml (Python 3.7)
Run           : python growth_sim_v18_asymm.py
...
class Ossicle:
    ...
    def plot_graph(self, fig, ax):
        ...
        ax.scatter([self.growth_center[0]], [self.growth_center[1]], color='black', s=800, zorder=4)   #800 size
        ...
        x_val = np.linspace(-5/3*rad_size, 5/3*rad_size, 400)   
        y_val = np.linspace(-5/3*rad_size, 5/3*rad_size, 400)
        ...
        cbar = ax.imshow(c_total, extent=(-5/3*rad_size, 5/3*rad_size, -5/3*rad_size, 5/3*rad_size), cmap=cmap, norm=norm, zorder=-1, alpha=0.9)
...
if __name__ == '__main__':
    ...
    # save_data = False
    # save_final_data = False
    base_folder = str(OUT_ROOT / "06_branching_lattice_model_2d" / "Network simulations" / "old tests" / "unique_morphologies_test")
